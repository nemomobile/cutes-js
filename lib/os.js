...
cp = function(src, dst, options) {
    var short_bool_options = { recursive: "r", force: "f", update: "u", deref: "L",
                         no_deref: "P" };
    var long_param_options = { preserve: "preserve", no_preserve: "no-preserve" };
    var cmd_options = [];

    options = options || {};
    util.eachProperty(options,function(n, v) {
        var opt = short_bool_options[n];
        if (opt !== undefined) {
            if (v)
                cmd_options.push(["-", opt].join(""));
            return;
        }
        var opt_name = long_param_options[n];
        if (opt_name !== undefined) {
            cmd_options.push(["--", opt_name, "=", v].join(""));
        }
    });
    return system("cp", cmd_options.concat([src, dst]));
};
...
cptree = function(src, dst, options) {
    ...
    if (options.preserve)
        cmd_options.push('--preserve=' + options.preserve);
    return system("cp", cmd_options.concat([src, dst]));
};
