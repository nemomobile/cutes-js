...
var mk_path = function() {
    ...
    that.mime = function(p) {
        ...
        if (name_type.length !== 2)
            error.raise({msg: "Unexpected file --mime-type result", expected: "name: type", got: s});
...
var stat = util.wrap(function(path, options) {
    ...
    var res = {}, i = 0;
    fields = options.filesystem ? that.filesystem_fields : that.fields;
...
var du = util.wrap(function(path, options) {
    ...
    var short_options = {
        summarize: "s", one_filesystem : "x"
    };

    var cmd_options = sys.command_line_options(options, that.short_options);
    cmd_options.push(path);
    ...
    short_options : { summarize: "s", one_filesystem : "x" }
    , default_options : { summarize: true, one_filesystem: true }
}, {
    params: {
