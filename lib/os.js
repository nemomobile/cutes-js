...
(function() {
    ...
    lib.os = {
        ...
        cptree : function(src, dst, dereference) {
            var opts = ["-rfu", src, dst]
            if (dereference)
                opts.push('-L')
            return lib.os.system("cp", opts)
        },
        /// update files with newer versions, dereference source links
        update_tree : function(src, dst) {
            return lib.os.system("cp", ["-rfuL", src, dst])
        },
        path : path,
