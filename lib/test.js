...
that.fixture = function() {
    ...
    teardown = function(fns) {
        for (var i = 0; i < fns.length; ++i) {
            try {
                ...
                debug.critical("Exception on teardown " + i + ":" + e);
            }
        }
    };
    ...
    fixture.execute = function() {
        ...
        if (fixture.is_failed)
            error.raise({message: "Test suite is failed"});
    };
    ...
    fixture.addTest = function(name, fn) {
        ...
        var execute = function() {
            try {
                ...
                debug.critical("Test '" + name + "' is failed.");
                debug.critical("Reason: " + e + "/" + parts.join(', '));
                debug.critical("Stack:" + e.stack ? e.stack : "???");
