...
var __slice = [].slice;
var dump, doc, wrap, cons, filter, config;
...
doc = (config.document ? doc_ : function(fn, doc) { return fn; });
...
var mapObject = doc(function(o, fn, options) {
    var res = [];
    options = options || {};
    var filter = (options.enum_fn
                  ? function(k, v) { return (typeof v !== "function"); }
                  : function(k, v) { return true; });
    var enumerate = options.deep ? eachPropertyDeep : eachProperty;
    enumerate(o, function(k, v) {
        if (filter(k, v))
            res.push(fn(k, v));
    });
    return res;
